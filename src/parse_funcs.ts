...
/*
export function npm_2_git(npmUrl: string, callback: (gitUrl: string) => void): void {
  // check if input is a valid URL
  if (!URL.parse(npmUrl).hostname) {
      throw new Error(`Invalid NPM package URL: ${npmUrl}`);
  }
  ...
  try {
      ...
      axios.get(`https://registry.npmjs.org/${packageName}`).then((response) => {
          ...
          // check if repository is on github
          if (isGitHubUrl(packageInfo.repository.url)) {
              callback(packageInfo.repository.url.replace("git+https", "git"));
          } else {
              throw new Error(
                  `Repository of package: ${packageName} is not on GitHub`
              );
...
/*
export function getGitRepoDetails(url: string): {username: string, repoName: string} | null {
    let match: RegExpMatchArray | null;
    console.log (`\nParsing -> ${url}\n`)
...
export async function fetchGithubAPI(gql_query: string): Promise<any> {
    try {
